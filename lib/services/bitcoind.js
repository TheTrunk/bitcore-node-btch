...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './BTCH.conf');

  log.info('Using BTCH config file:', configPath);
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('btch Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
